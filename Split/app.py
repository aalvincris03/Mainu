...
def split():
    ...
    reason = data['reason']
    lender = data['lender']
    amount_each = total / len(names)
    for name in names:
        debt = Debt(date=date, name=name, amount=amount_each, reason=reason, lender=lender, status=False)
        db.session.add(debt)
        log = History(action='Split', detail=f'{name} owes {amount_each:.2f} for {reason} from {lender}', timestamp=datetime.now())
        db.session.add(log)
    db.session.commit()
    return jsonify({'message': 'Split added'})
